...
if docker:
    ...
    GROUND_TRUTH_NUCLEI_DIRECTORY = Path("/opt/app/ground_truth_nuclei_full_3")
    GROUND_TRUTH_TISSUE_DIRECTORY = Path("/opt/app/ground_truth_tissue_full")
    metrics_tissue_output = '/output/metrics_tissue.json'
else:
    INPUT_DIRECTORY = Path("test")
    OUTPUT_DIRECTORY = Path("output")
    GROUND_TRUTH_NUCLEI_DIRECTORY = Path("ground_truth_nuclei_full_3")
    GROUND_TRUTH_TISSUE_DIRECTORY = Path("ground_truth_tissue_full")
    metrics_tissue_output = 'output/metrics_tissue.json'
...
def process(job):
    ...
    location_nuclei_json = get_file_location(
            job_pk=job["pk"],
            values=job["outputs"],
            slug="melanoma-3-class-nuclei-segmentation",
        )
...
def main():
    ...
    metrics["aggregates"]["macro_f1_nuclei"] = macro_f1
